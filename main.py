...
bot = telebot.TeleBot("5861397527:AAHdDXcE-Z5EN5VhW73ygeIx3-jL5OSfucg")
                ...
                if int(d.day) <= int(now.day):
                    if int(tim1[0]) <= int(int(now.hour)):
                        if int(tim1[1]) <= int(int(now.minute)):
                            ...
                            gs = GoogleSheet()
                            range = f'Вох1 (копия) (копия) (копия)!{t}:{chr(ord("@") + k)}{t}'
                            values = [
                                [""]
                            ]
...
def start(message):
    ...
    bot.send_message(message.chat.id,
                     f'{datetime.datetime.now()}Автосервис «Два товарища» осуществляет полный комплекс услуг по ремонту и техническому обслуживанию, необходимые Вашему автомобилю.\nМы работаем с автомобилями отечественного и импортного производства любых марок и разных моделей.\nНаш автосервис имеет слаженную команду профессионалов, отлично знающих свою работу.\n✅ Наш виртуальный помощник подберет для вас удобное время. \n✅ Выберите то что вам подходит из предложенных вариантов.\n✅ По окончанию диалога, вы будете записаны.',
                     reply_markup=dele)
...
def text_save3(message):
    ...
    if not creds or not creds.valid:
        ...
        with open('token.json', 'w') as token:
            token.write(creds.to_json())
    SAMPLE_RANGE_NAME = 'Вох1 (копия)!B3:ZZ'
    try:
        service = build('sheets', 'v4', credentials=creds)
        sheet = service.spreadsheets()
        result = sheet.values().get(spreadsheetId=SAMPLE_SPREADSHEET_ID,
...
def text_save4(message):
    ...
    if not creds or not creds.valid:
        ...
        with open('token.json', 'w') as token:
            token.write(creds.to_json())
    SAMPLE_RANGE_NAME = f'Вох1 (копия) (копия) (копия)!4:100'
    try:
        service = build('sheets', 'v4', credentials=creds)
        sheet = service.spreadsheets()
        result = sheet.values().get(spreadsheetId=SAMPLE_SPREADSHEET_ID,
        ...
        for row in values:
            try:
                ...
                treug = types.KeyboardButton('%s' % row[0])
                markup.add(treug)
    except HttpError as err:
        bot.send_message(message.chat.id, err)
...
def text_save5(message):
    ...
    if message.text == 'Нaзaд' and back == 1:
        ...
        if not creds or not creds.valid:
            ...
            with open('token.json', 'w') as token:
                token.write(creds.to_json())
        SAMPLE_RANGE_NAME = 'Вох1 (копия) (копия) (копия)!4:100'
        try:
            service = build('sheets', 'v4', credentials=creds)
            sheet = service.spreadsheets()
            result = sheet.values().get(spreadsheetId=SAMPLE_SPREADSHEET_ID,
...
def text_save6(message):
    ...
    if message.text == 'Подтвердить запись':
        ...
        bot.send_message(message.chat.id,
                         f'Спасибо, что выбрали нас! Ждем Вас в {day} в {tim} Г. НИЖНИЙ НОВГОРОД, СОРМОВСКИЙ РАЙОН, УЛ. КОНОВАЛОВА, 15А Тел.8-903-600-41-19 (СЕРГЕЙ)',
                         reply_markup=markup)
        day1 = str(day).split()
        dat = day1[1].split('.')
        tim1 = tim.split(':')
